...
class imap_pop {
	...
	*/
	...
	function delete_email($id = 0)
	{
		...
		return imap_delete($this->IMAP_resource, intval($this->msg_id));
	}
	...
	*/
	function grab_email_as_array($id = 0)
	{
		...
		if (isset($structure->disposition) && $structure->disposition == 'INLINE') {
		
			$this->extract_a_part($structure,1);
		...
		{ 
			...
			if (strlen($text)) $this->parts_array['not multipart']['text'] = array('type'=>$structure->subtype,'string'=>$text);
		}
		//p($this->parts_array);
		//exit();
	...
	*/ 
	function close($delete=false)
	{
		...
		{
			if ($delete) $closed = imap_close($this->IMAP_resource,CL_EXPUNGE);
			else $closed = imap_close($this->IMAP_resource);
