...
class Piwik {
	...
	private static $auth_token = 'f1dbd0384fb510ca2e76ff64c7f5fb34';
	
	private static $api_urls = array(
		'31.193.177.27' => 'analytics.fw4.be'
	);
	
	private static $tracker = false;
	private static $tracked_page_view = false;
	
	private static function get_site_id() {
		$site = current_site();
		if ($site->piwik_id) return $site->piwik_id;
		...
		if (isset($result->row->idsite)) $site_id = $result->row->idsite;
		else $site_id = self::create_site();
	...
	private static function get_tracker() {
		if (!self::$tracker) self::$tracker = new PiwikTracker(self::get_site_id(), 'http://'.self::$api_urls[$_SERVER['SERVER_ADDR']].'/');
		return self::$tracker;
	}
	...
	public static function track_page_view() {
		...
		if (!$site->live || !isset(self::$api_urls[$_SERVER['SERVER_ADDR']])) return false;
		
		$tracker = self::get_tracker();
		$tracker->getUrlTrackPageView();
		self::$tracked_page_view = true;
	}
	
	public static function process() {
		if (self::$tracker) {
			$tracker = self::get_tracker();
			if (self::$tracked_page_view) $tracker->doTrackPageView(Router::$document_title);
		}
	}
