...
class phpthumb {
	...
	var $config_imagemagick_path                     = '/usr/local/bin/convert';
				...
				if (is_array($getimagesize)) {
					...
					if ($this->source_width || $this->source_height) {
						if ($this->zc) {
							...
							/*
							if ($wAll > $this->source_width) {
								$hAll = round($hAll * ($this->source_width / $wAll));
								$wAll = $this->source_width;
							}
							if ($hAll > $this->source_height) {
								$wAll = round($wAll * ($this->source_height / $hAll));
								$hAll = $this->source_height;
							}
							*/
							...
							if ($imAR < $zcAR) {
								$commandline .= ' -'.$IMresizeParameter.' '.escapeshellarg($wAll.'x');
							} else {
								$commandline .= ' -'.$IMresizeParameter.escapeshellarg(' x'.$hAll);
							}
